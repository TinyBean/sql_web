...
description: 使用固定 LOT、PCIe 平台排除、MT/ST、Machine_Running、Performance (DUT-On)、Performance (Test Time)、Yield 和业务日聚合规则，以可信 SQLite 数据链路查询、计算或解释 Test OEE。适用于默认口径及明确的临时口径调整；不适用于 Assembly OEE。
---
...
5. 默认 SQL 在 `业务日 + MT/ST` 粒度分别汇总 Availability、Performance (DUT-On)、Performance (Test Time) 和 Yield，以 Availability 日结果为主左连接 DUT 日结果，再将四项相乘得到日 OEE。Availability 分母中的机台数是当天该类型具有有效 Availability 记录的不重复机台数；即使某台机全是 loss，也必须保留在分母中。多日 OEE 是可计算日 OEE 的等权平均。Test Time 按同日同类型 0.2% 截尾均值×TD次数÷实际秒数计算，每端截去 floor(n/1000) 条。最终多日值读取 `period_test_oee`，不要对这个重复展示在每日行上的值再次求和或平均。
...
- `test_oee_calculator__get_default_sql`：生成完整的默认 SQLite 查询。PCIe 排除优先；结果包含 Availability、Performance (DUT-On)、Performance (Test Time)、Yield、逐日与多日 Test OEE 及覆盖计数。
