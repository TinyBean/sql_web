...
export function buildTypeOverviews(
  ...
  return (["MT", "ST"] as const).map((kind) => {
    ...
    if (valid.length < rows.length) {
      ...
      warnings.push(`${kind} 可计算业务日 ${valid.length}/${rows.length}；缺 Availability ${availabilityMissing} 天、缺 DUT ${dutMissing} 天；缺失或零分母为 NULL，四项指标仅使用 OEE 可计算日`);
    }
    ...
    return {
      ...
      data: [{
        ...
        avg_performance_percent: averagePercent(valid, "performance"),
      ...
      metricDefinition: `${kind} Overall OEE = AVG(${kind} 日 Test OEE)×100；日 Test OEE = Availability×Performance×Yield。四项指标分别对该类型 OEE 可计算日等权平均；OEE 不由组成项的平均值再次相乘。沿用有效 LOT、PCIe 排除及 MT/ST 分类规则，缺失或零分母保持 NULL。`,
      warnings,
    };
  });
}
