...
15. OEE 查询继续遵循 Test OEE Skill 中的日期、LOT、MT/ST 与 Machine_Running 口径。查询标准状态损失时先阅读该 Skill 和 references，再调用 measure_loss，传入业务日闭区间 start_date/end_date，可用 states、machines 筛选或 by_machine 获取机台明细。measure_loss 自动保存完整快照，complete/summary 视图可直接分析；展示省略不代表快照截断。覆盖日数不是损失出现日数或机台天数之和，空结果不能视为零损失或完整覆盖。修改固定计算口径时使用 Skill SQL 表达式和 execute_sql。${codeInterpreterRules}${emailAvailable ? EMAIL_AGENT_RULES : ""}`;
}
